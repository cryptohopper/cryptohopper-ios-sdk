...
class TournamentAPIV2Spec : QuickSpec {
    ...
    override func spec() {
        ...
        context("V2 Tournament"){
            
            it("Get All Tournaments"){
                waitUntil(timeout: apiTimeout) { done in
                    CryptohopperV2Tournament.getTournaments(completion: { result in
                        switch(result){
                        case .success(let signals):
                            expect(signals).to(beAKindOf([V2Tournament].self))
