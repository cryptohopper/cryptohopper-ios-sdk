...
@objc public class CryptohopperV2Onboarding: NSObject {
    ...
    public static func createOnboardingHopper(hopperId : Int, isBuyingEnabled : Bool , isSellingEnabled : Bool , isHopperEnabled : Bool,isPortfolioSyncEnabled: Bool, isPaperTrading : Bool , quoteCurrency : String, signallerId : Int? , riskLevel : String?,botType : Int?,portfolioBotId : Int?, completion: @escaping (Result<String?, Error>) -> Void) {
        HopperAPIV2OnboardingCreateRequest.init(hopperId: hopperId, isBuyingEnabled: isBuyingEnabled, isSellingEnabled: isSellingEnabled, isHopperEnabled: isHopperEnabled,isPortfolioSyncEnabled : isPortfolioSyncEnabled, isPaperTrading: isPaperTrading, quoteCurrency: quoteCurrency, signallerId: signallerId, riskLevel: riskLevel, portfolioBotId: portfolioBotId, botType: botType).request { (_) in
            completion(.success("Success"))
        } _: { (err) in
            completion(.failure(err))
        }
